...
User Function BFMA46HS()
	...
	Private oFWLayer
	Private INCLUI    := .T.
    Private cTit46HS  := '(' + Iif('SC' $ cMVYOISCPP, 'Solicitação de Compra, ', '') + Iif('SP' $ cMVYOISCPP, 'Solicitação de Produto, ', '') + Iif('PC' $ cMVYOISCPP, 'Pedido de Compra', '') + Iif('MC' $ cMVYOISCPP, 'Medição de Contrato', '') + ')'
	...
	Private nVlrDsc   := 0
	Private nTotPed   := 0
	Private cFilCtr   := cFilAnt // Variável usada na Consulta de Medições de Contrato e na Geração de Medição de Contrato
	...
	If !Empty(cNum46HS)
		...
		oTpRequis := TComboBox():New(Dim(008), Dim(015), {|u| if(PCount() > 0, cTpRequis := u, cTpRequis)}, aTpRequis, Dim(100),Dim(14), oPanelUp,,/*bValid*/,,,,.T.,,,,,,,,,'cTpRequis','Tipo de Requisição',1)
    	oTpRequis:bWhen := {|| .T.}
		If Len(aTpRequis) == 1
			oTpRequis:Select(1)
		EndIf
		...
		oRevisao := TGet():New(Dim(008), Dim(235), {|u| If(PCount() > 0, cRevisa := u, cRevisa)}, oPanelUp, Dim(030), Dim(010),PesqPict("CN9","CN9_REVISA"), /*bValid*/,CLR_BLACK,CLR_WHITE,,,,.T.,"",,,.F.,.F.,,.F.,.F., "", "cRevisa",,,,.T.,.F.,, "Revisão", 1)
		oRevisao:bWhen   := {|| .F. }
	...
	If .not. IsInCallStack('CONSULTA')
		...
		If p_cField = 'TIPOREQ'
			...
			ElseIf cTpRequis == 'MC'
				...
				// Analisa se o usuário tem permissão para Incluir Medição de Contratos
		...
		ElseIf p_cField = 'CONTRATO'
			...
			lVldVig	:= SuperGetMV("MV_CNFVIGE", .F., "N") == "N" //S=permite incluir fora da vigência,N=não permite.
			...
			If !Empty(cContrato)
				...
				If !(CNTVldCTR(cContrato, @cFilCtr) .And. CN240VldUsr(cContrato,DEF_TRAINC_MED, .T.,, cFilCtr))
					lRet := .F.
				EndIf

				If lRet .and. ( lRet := CN9->(dbSeek(xFilial("CN9",cFilCTR)+cContrato)) )		
					CN9->(DbSetOrder(7)) // CN9_FILIAL + CN9_NUMERO + CN9_SITUAC
					If CN9->(DbSeek(xFilial("CN9",cFilCtr) + cContrato + DEF_SVIGE))//-- busca revisão vigente
						If !Empty(CN9->CN9_REVATU)
							lRet := .F.				
							Help("",1,"CNTA120_REV","","Contrato não está vigente, verifique!",1,0) //- Valida se o contrato esta em vigencia
						Else
				...
				/*Else
					...
					Help(,, 'Codigo do Contrato',, "É obrigatório informar o Codigo do Contrato, quando o tipo de Requisição é Medição de Contrato!",1,0,,,,,,{'Informe o codigo do Contrato!'})
					lRet := .F.
		...
		ElseIf p_cField = 'CODFORN'
			...
			If Empty(cCodForn) 
				...
				ElseIf cTpRequis == 'MC'
					...
					Help(,, 'Codigo do Fornecedor',, "É obrigatório informar o Codigo do Fornecedor, quando o tipo de Requisição é Medição de Contrato!",1,0,,,,,,{'Informe o codigo do Fornecedor!'})
					lRet := .F.
		...
		ElseIf p_cField = 'LOJFORN'
			...
			If Empty(cLojForn) 
				...
				ElseIf cTpRequis == 'MC'
					...
					Help(,, 'Loja do Fornecedor',, "É obrigatório informar a Loja do Fornecedor, quando o tipo de Requisição é Medição de Contrato!",1,0,,,,,,{'Informe a Loja do Fornecedor!'})
					lRet := .F.
...
Static Function HeaderCons(p_lDefault)
	...
	aAdd(aRet, {"(-) Qtd. MC", "QTDMC", GetSX3Cache("TL_QUANTID", 'X3_PICTURE'), TamSX3('TL_QUANTID')[1], TamSX3('TL_QUANTID')[2], "AllWaysTrue()", "û", GetSX3Cache("TL_QUANTID", 'X3_TIPO'), "", "R",,,,"V"})
	...
	aAdd(aRet, {"Vl. Unit.", "VLUNIT", GetSX3Cache("C7_PRECO", 'X3_PICTURE'), TamSX3('C7_PRECO')[1], TamSX3('C7_PRECO')[2], "U_MA46HSVF('VLUNIT', M->VLUNIT)", "û", GetSX3Cache("C7_PRECO", 'X3_TIPO'), "", "R",,,"cTpRequis $ 'PC/MC' .and. oGridReq:GetQtdLinha() > 0 .and. oGridReq:Marcado()","R"})
	...
	aAdd(aRet, {"Grp. Aprovador", "GRPAPRO", GetSX3Cache("ZFA_XGRAPR", 'X3_PICTURE'), TamSX3('ZFA_XGRAPR')[1], TamSX3('ZFA_XGRAPR')[2], "U_MA46HSVF('GRPAPRO', M->GRPAPRO)", "û", GetSX3Cache("ZFA_XGRAPR", 'X3_TIPO'), "ZBJSP", "R",,,"oGridReq:GetQtdLinha() > 0 .and. oGridReq:Marcado() .and. cTpRequis <> 'MC'","A"})
...
static Function VldGravacao()
	...
	ElseIf cTpRequis == 'MC'
		...
		If Empty(cContrato)
			cMsgValid := 'É obrigatório informar o Contrato, quando o tipo de Requisição é Medição de Contrato!'
		EndIf

		If Empty(cCodForn)
			cMsgValid := 'É obrigatório informar o Codigo do Fornecedor, quando o tipo de Requisição é Medição de Contrato!'
		EndIf

		If Empty(cLojForn)
			cMsgValid := 'É obrigatório informar a Loja do Fornecedor, quando o tipo de Requisição é Medição de Contrato!'
		EndIf
...
@description Gera Medição de Contrato
@type function
@version 12.1.2210
@author Helitom Silva
@since 21/02/2024
...
Static Function GerMedContrato()
	...
	Begin Sequence
		...
		CN9->(DbSetOrder(1))
		If CN9->(DbSeek(xFilial("CN9", cFilCtr) + cContrato)) //Posicionar na CN9 para realizar a inclusão
			...
			If (oModel:CanActivate())
				...
				/*
				oModel:LoadValue("CNDMASTER", "CND_RCCOMP", "1") // Selecionar competência
			...
			If lRet
				...
				If MsgYesNo('Medição gerada com sucesso sob Nº ' + cNumMed + CRLF + CRLF + 'Deseja encerrar esta medição gerando seu pedido de compras?', 'Medição de Contrato')

					lRet := CN121Encerr(.T.) //Realiza o encerramento da medição

					If lRet .and. Select('SC7') > 0
						MsgInfo("Medição de Contrato: " + cNumMed + " - Pedido de Compra: " + SC7->C7_NUM + " incluído com sucesso!")
					ElseIf lRet
						MsgInfo("Pedigo de compras não encontrado, verifique!", 'Medição de Contrato')
					EndIf
...
Static Function GetOperacoes()
	...
	For nX := 1 to Len(aMVYOISCPP)
		...
		ElseIf aMVYOISCPP[nX] $ 'MC'
			...
			Aadd(aRet, 'MC=Medição de Contrato')
